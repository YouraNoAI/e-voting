...
class Admin extends CI_Controller {
	...
	public function voting(){
		...
		if ($data['cekvoting'] > 0) {
			...
			$data['sudah_memilih'] = $this->db->query('SELECT pemilih.id_pemilih, pemilih.nim, pemilih.nama, pemilih.angkatan, ikut_voting.waktu FROM pemilih JOIN ikut_voting ON pemilih.id_pemilih=ikut_voting.id_pemilih JOIN voting ON ikut_voting.id_voting=voting.id_voting WHERE pemilih.id_pemilih IN (SELECT ikut_voting.id_pemilih FROM ikut_voting)')->result();

			$data['belum_memilih'] = $this->db->query('SELECT id_pemilih,nim, nama,angkatan FROM pemilih WHERE id_pemilih NOT IN (SELECT id_pemilih FROM ikut_voting)')->result();
		}
	...
	function tambah_voting(){
		...
		$data = ['nama_voting' => $voting,'status_voting'=>'nonaktif'];
	...
	function edit_voting($id){
		$voting = $this->input->post('voting');
		$status_voting = $this->input->post('status_voting');
		$data = ['nama_voting' => $voting,'status_voting' => $status_voting];
	...
	function edit_kandidat(){
		...
		if (empty($foto['name'])) {
			$data = ['nama_kandidat' => $nama, 'semester' => $semester,'angkatan' => $angkatan,'visi' => $visi,'misi' => $misi];
	...
	function get_pemilih($id){
		...
		$d = $this->db->select('id_pemilih, nim,nama,angkatan, username')->get_where('pemilih', ['id_pemilih' => $id])->row();
		echo json_encode($d);
	}
	function tambah_pemilih(){
		...
		$data 		= ['nim' => $nim,'nama' => $nama,'angkatan' => $angkatan, 'username' => $username, 'password' => $password];
	...
	function edit_pemilih($id){
		...
		$username 	= $this->input->post('username');
		$where 		= ['id_pemilih' => $id];
		$data 		= ['nim' => $nim,'nama' => $nama,'angkatan' => $angkatan, 'username' => $username];
